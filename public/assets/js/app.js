...
(function () {
	...
	function twoColumnMenuGenerate() {
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			...
			currentPath = (currentPath.length > 1 && currentPath[1] != "tambah") ? currentPath[0]+ '-'+currentPath[1] : currentPath[0];
			if (currentPath) {
				// navbar-nav
				var a = document.getElementById("navbar-nav").querySelector('[data-key="t-'+ currentPath +'"]');
	...
	function initTwoColumnActiveMenu() {
		...
		if (currentPath) {
			if(document.body.className == "twocolumn-panel"){
				document.getElementById("two-column-menu").querySelector('[data-key="t-'+ currentPath +'"]').classList.add("active");
			}
			// navbar-nav
			var a = document.getElementById("navbar-nav").querySelector('[data-key="t-'+ currentPath +'"]');
			if (a) {
	...
	// two-column sidebar active js INI YANG BENARRRR
	function initActiveMenu() {
		var currentPath = location.pathname == "/" ? "/" : location.pathname.substring(1);
		currentPath = (currentPath == "/") ? "/" : currentPath.split('/');
		console.log(currentPath[1]);
		currentPath = (currentPath.length > 1 && currentPath[1] !== 'tambah' && currentPath[1] !== 'edit' && currentPath[1] !== 'new') ? currentPath[0]+ '-'+currentPath[1] : currentPath[0];
